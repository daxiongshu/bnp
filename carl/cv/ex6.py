...
clf=ExtraTreesClassifier(n_estimators=850,max_features= 40,criterion= 'entropy',min_samples_split= 4,
                            verbose=1,max_depth= 30, min_samples_leaf= 3, n_jobs = -1)

clf.fit(X,y)
idx=np.array(test.index)#id_test
yp=clf.predict_proba(Xt).T[1]
s=pd.DataFrame({idname:idx,'PredictedProb':yp})
s.to_csv('ex6.csv',index=False)
