...
idname='ID'
labelname='target'
train=pd.read_csv('../tian/clean/train_clean2.csv',index_col=idname).replace(-999,-1)
y=np.array(train[labelname]).astype(float)
train.drop([labelname],inplace=True,axis=1)
X=train.as_matrix()
idx=np.array(train.index)
train=pd.read_csv('../tian/clean/test_clean2.csv',index_col=idname).replace(-999,-1)
train.drop([labelname],inplace=True,axis=1)
Xt=train.as_matrix()
idx=np.array(train.index)
