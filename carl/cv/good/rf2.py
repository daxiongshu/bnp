...
def kfold_cv(X_train, y_train,idx,k):
    ...
    for train_index, test_index in kf:
        ...
        #for j in range(m):
        #    clf=xgb_classifier(eta=0.1,min_child_weight=20,col=0.7,subsample=1,depth=5,num_round=50,seed=j*77,gamma=0.1)
        #    y_pred+=clf.train_predict(X_train_cv,(y_train_cv),X_test_cv,y_test=(y_test_cv))
        #y_pred/=m;
        clf=RandomForestClassifier(n_jobs=-1,n_estimators=100,max_depth=20)
...
del train
gc.collect()
#X1, _ = get_data('../hash/rebuild1/train4.svm')
#X=sparse.hstack([X,X1,pickle.load(open('../sparse/X.p'))],format='csr')
#tmp=pd.read_csv('../google/master/test/pred.csv')
#X=np.vstack((X.T,np.array(tmp['is_screener']))).T
#X=np.hstack([X,loadsrk()])
#X=np.vstack((X.T,loadsvm2(y))).T
