...
idname='ID'
labelname='target'
train=pd.read_csv('../tian/clean/train_clean1.csv',index_col=idname).replace(-999,-1)
test=pd.read_csv('../tian/clean/test_clean1.csv',index_col=idname).replace(-999,-1)

y=np.array(train[labelname]).astype(float)
train.drop([labelname],inplace=True,axis=1)
test.drop([labelname],inplace=True,axis=1)
X=train.as_matrix()
Xt=test.as_matrix()
