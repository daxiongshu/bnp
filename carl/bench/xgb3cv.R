...
# Read train and test
train_raw <- fread("../input/tr.csv", stringsAsFactors=TRUE) 
print(dim(train_raw))
...
test_raw <- fread("../input/va.csv", stringsAsFactors=TRUE) 
test_id <- test_raw$ID
test_raw$ID <- NULL
print(dim(test_raw))
...
submission <- read.csv("tree1cv.csv")
...
# Prepare submission
write.csv(submission, "bnp-xgb-ks10-cv.csv", row.names=F, quote=F)
