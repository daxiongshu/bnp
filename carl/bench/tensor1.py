...
def onehot(y):
    ...
    for c,i in enumerate(tmp):
        yp[y==i,c]=1
    return yp
def process_data2(path, is_test_set=False):
    df = pd.read_csv(path)
    if not is_test_set:
...
weights = [
    tf.Variable(tf.random_normal([131, 50], seed=888)),
...
with tf.Session() as sess:
    ...
    X_train, X_test, y_train, y_test = process_data2(train_path)
    ...
    for step in range(401):
        ...
        if step%100 == 0:
            ...
            for v_step in range(len(X_test)//BATCH_SIZE):
                ...
                #print (type(batch_data_v),type(batch_labels_v))
    ...
    X_eval, ids = process_data2(test_path, True) # '/Users/Peace/Projects/Cardif/data/raw/test.csv'
