...
train = train.drop(['ID','target']+bad,axis=1)
test = pd.read_csv("../input/va.csv")
id_test = test['ID'].values
test = test.drop(['ID']+bad,axis=1)
...
extc = RandomForestClassifier(n_estimators=100,max_features= 40,criterion= 'entropy',min_samples_split= 4,verbose=1,n_jobs=-1,
...
pd.DataFrame({"ID": id_test, "PredictedProb": y_pred[:,1]}).to_csv('rf3cv.csv',index=False)
