...
bad=["v8","v23","v25","v36","v37","v46",
                      ...
                      "v119","v123","v124","v128"]
print('Load data...')
train = pd.read_csv("../input/tr.csv").drop(bad,axis=1)
target = train['target'].values
train = train.drop(['ID','target'],axis=1)
test = pd.read_csv("../input/va.csv").drop(bad,axis=1)
...
extc = ExtraTreesClassifier(n_estimators=700,max_features= 50,criterion= 'entropy',min_samples_split= 4,
                            max_depth= 50, min_samples_leaf= 4, verbose=1,n_jobs=-1)      
...
pd.DataFrame({"ID": id_test, "PredictedProb": y_pred[:,1]}).to_csv('ex3cv.csv',index=False)
