...
def kfold_cv(X_train, y_train,idx,k):
    ...
    for train_index, test_index in kf:
        ...
        for j in range(m):
            clf=xgb_classifier(eta=0.01,min_child_weight=2,col=0.7,subsample=0.68,depth=5,num_round=500,seed=j*77,gamma=0)
...
#X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv481819819778.csv',index_col=idname)[labelname].values]).T
