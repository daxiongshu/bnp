...
train.replace(-999,-1,inplace=True)
...
test.replace(-999,-1,inplace=True)
...
X=np.vstack([trainid,y,X.T]).T
print X.shape
s=pd.DataFrame(X,columns=['ID','target']+['fea%d'%i for i in range(X.shape[1]-2)])
s.to_csv('stack_train2.csv',index=False)
Xt=np.vstack([testid,Xt.T]).T
s=pd.DataFrame(Xt,columns=['ID']+['fea%d'%i for i in range(Xt.shape[1]-1)])
s.to_csv('stack_test2.csv',index=False)
