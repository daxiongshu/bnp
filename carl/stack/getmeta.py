...
s=pd.DataFrame(np.vstack([trainid,X.T]).T,columns=['ID']+good+['metafea%d'%i for i in range(X.shape[1]-len(good))])
s.to_csv('meta-train.csv',index=False)
del X,s
import gc 
gc.collect()
#Xt=sparse.hstack([Xt,Xt_sparse],format='csr').toarray()
s=pd.DataFrame(np.vstack([testid,Xt.T]).T,columns=['ID']+good+['metafea%d'%i for i in range(Xt.shape[1]-len(good))])
