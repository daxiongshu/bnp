...
X=np.vstack([X.T,pd.read_csv('../cv/good/xgb10.csv',index_col=idname)[labelname].values]).T
...
Xt=np.vstack([Xt.T,pd.read_csv('../cv/xgb10.csv',index_col=idname)[labelname].values]).T
...
Xt=np.vstack([Xt.T,pd.read_csv('../bench/ex4.csv',index_col=idname)['PredictedProb'].values]).T
...
s.to_csv('stack15.csv',index=False)
