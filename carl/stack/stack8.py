...
X=np.vstack([X.T,pd.read_csv('../bench/bnp-xgb-ks10-cv.csv',index_col=idname)['PredictedProb'].values]).T
...
Xt=np.vstack([Xt.T,pd.read_csv('../bench/bnp-xgb-ks10.csv',index_col=idname)['PredictedProb'].values]).T
print X.shape,y.shape,Xt.shape
X=sparse.hstack([X,X_sparse],format='csr').toarray()
Xt=sparse.hstack([Xt,Xt_sparse],format='csr').toarray()
print X.shape,y.shape,Xt.shape
clf=xgb_classifier(eta=0.01,min_child_weight=1,col=0.7,subsample=0.68,depth=5,num_round=500,gamma=0)
assert(X.shape[1]==Xt.shape[1])
yp=clf.train_predict(X,y,Xt)
s=pd.DataFrame({idname:idx,'PredictedProb':yp})
s.to_csv('stack8.csv',index=False)
