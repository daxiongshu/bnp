...
tiantrain=pd.read_csv('../tian/tian_train.csv',index_col=idname)
good1=[]
for i in tiantrain.columns.values:
    train[i]=tiantrain[i]
    if i!='target' and 'count' in i:#and train[i].dtype!='0':
        print 'good1',i
        good1.append(i)

test=pd.read_csv('../cv/test_clean1.csv',index_col=idname)
tiantest=pd.read_csv('../tian/tian_test.csv',index_col=idname)
for i in tiantest.columns.values:
    test[i]=tiantest[i]
...
for name in train.columns.values :
    if train[name].value_counts().shape[0]<100 and 'count' not in name and name not in badx:
...
s.to_csv('stack20.csv',index=False)
