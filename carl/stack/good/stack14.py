...
def kfold_cv(X_train, y_train,idx,k):
    ...
    for train_index, test_index in kf:
        ...
        for j in range(m):
            clf=xgb_classifier(eta=0.01,min_child_weight=2,col=0.6,subsample=0.68,depth=5,num_round=600,seed=j*77,gamma=0)
...
vec = DictVectorizer()
names_categorical = []
for name in good:#train.columns.values :
    train[name]=(train[name]*10).astype(int)
    if train[name].value_counts().shape[0]<100 and name not in badx:
...
X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv460224349494.csv',index_col=idname)[labelname].values]).T
#X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv497213179983.csv',index_col=idname)[labelname].values]).T
...
print X.shape,y.shape
X=sparse.hstack([X[:,-8:],X_sparse],format='csr')#.toarray()
print X.shape,y.shape
xx=[]
