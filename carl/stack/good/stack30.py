...
def kfold_cv(X_train, y_train,idx,k):
    ...
    for train_index, test_index in kf:
        ...
        for j in range(m):
            clf=xgb_classifier(eta=0.01,min_child_weight=2,col=0.7,subsample=0.8,depth=5,num_round=500,seed=j*77,gamma=0)
...
#X=train.drop(names_categorical,axis=1).as_matrix()
...
#X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv507091142977.csv',index_col=idname)[labelname].values]).T
#X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv508310195402.csv',index_col=idname)[labelname].values]).T
#X=np.vstack([X.T,pd.read_csv('../cv/vabackup/mycv511753946977.csv',index_col=idname)[labelname].values]).T
