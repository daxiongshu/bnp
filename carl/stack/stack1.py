...
idx=np.array(train.index)
Xt=train.as_matrix()
Xt=np.vstack([Xt.T,pd.read_csv('../cv/xgb1.csv',index_col=idname).as_matrix().ravel()]).T
Xt=np.vstack([Xt.T,pd.read_csv('../cv/rf1.csv',index_col=idname).as_matrix().ravel()]).T
...
s.to_csv('stack1.csv',index=False)
