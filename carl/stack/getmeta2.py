...
#s=pd.DataFrame(np.vstack([trainid,X.T]).T,columns=['ID']+['fea%d'%i for i in range(X.shape[1])])
#s.to_csv('meta-train.csv',index=False)
from sklearn.datasets import dump_svmlight_file
dump_svmlight_file(X,y,'meta-train-sparse.svm')

import gc 
gc.collect()
Xt=sparse.hstack([Xt,Xt_sparse],format='csr').toarray()
dump_svmlight_file(Xt,np.zeros(Xt.shape[0]),'meta-test-sparse.svm')
#s=pd.DataFrame(np.vstack([testid,Xt.T]).T,columns=['ID']+['fea%d'%i for i in range(Xt.shape[1])])
#s.to_csv('meta-test.csv',index=False)
